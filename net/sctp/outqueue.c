...
/* Trigger rescheduling per packet */
#define CMT_PACKET
/* Trigger rescheduling per CWND */
//#define CMT_CWND

/* Selects transport using pure RR */
//#define CMT_SELECT_RR
/* Selects transport using RR but only those with cwnd available */
#define CMT_SELECT_RR_CWND
/* Selects transport with the highest cwnd still available */
...
#elif defined(CMT_SELECT_RR_CWND)
		if (t->cwnd > t->flight_size) {
			new_transport = t;
			break;
		}
